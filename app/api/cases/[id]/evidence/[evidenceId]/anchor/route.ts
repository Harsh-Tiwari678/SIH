...
// this route operates on); the segment is named after the evidence route it
// now lives under. The version's own case is what authorizes.
//
...
export async function POST(
  _request: Request,
  ctx: RouteContext<"/api/cases/[id]/evidence/[evidenceId]/anchor">,
) {
  ...
  let outcome;
  try {
    outcome = await anchorDocumentVersion(evidenceId);
  } catch (raw) {
